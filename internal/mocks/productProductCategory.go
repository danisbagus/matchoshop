// Code generated by mockery v2.10.0. DO NOT EDIT.

package mocks

import (
	errs "github.com/danisbagus/go-common-packages/errs"
	domain "github.com/danisbagus/matchoshop/internal/core/domain"

	mock "github.com/stretchr/testify/mock"
)

// IProductProductCategoryRepo is an autogenerated mock type for the IProductProductCategoryRepo type
type IProductProductCategoryRepo struct {
	mock.Mock
}

// BulkInsert provides a mock function with given fields: data
func (_m *IProductProductCategoryRepo) BulkInsert(data []domain.ProductProductCategory) *errs.AppError {
	ret := _m.Called(data)

	var r0 *errs.AppError
	if rf, ok := ret.Get(0).(func([]domain.ProductProductCategory) *errs.AppError); ok {
		r0 = rf(data)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*errs.AppError)
		}
	}

	return r0
}

// DeleteAll provides a mock function with given fields: productID
func (_m *IProductProductCategoryRepo) DeleteAll(productID int64) *errs.AppError {
	ret := _m.Called(productID)

	var r0 *errs.AppError
	if rf, ok := ret.Get(0).(func(int64) *errs.AppError); ok {
		r0 = rf(productID)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*errs.AppError)
		}
	}

	return r0
}
